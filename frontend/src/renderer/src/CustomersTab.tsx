...
import { Search, Plus, Users, X, UserCircle, RefreshCw, Mail, Phone, CreditCard, ShoppingBag } from 'lucide-react'
                    ...
                    {paginated.length === 0 ? (
                       <tr>
                          <td colSpan={4} className="px-6 py-16 text-center text-zinc-500">
                             ...
                             <p className="text-sm mt-1">{query.trim() ? 'Intenta con otra búsqueda.' : 'Haz clic en "Nuevo Cliente".'}</p>
                          </td>
                       </tr>
                    ) : (
                       paginated.map(c => (
                          <tr
                             key={String(c.id)}
                             onClick={() => { selectCustomer(c); setIsDrawerOpen(true); }}
                          ...
                          >
                             <td className="px-6 py-4 font-medium text-zinc-200">
                                <div className="flex items-center gap-3">
                                   ...
                                   <div className="truncate group-hover:text-emerald-400 transition-colors">{c.name}</div>
                                </div>
                             </td>
            ...
            >
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2 flex items-center gap-1.5"><Phone className="w-3.5 h-3.5"/> Teléfono</label>
                        <input
                     ...
                     <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5"/> Correo</label>
                        <input
                     ...
                     <>
                        ...
                        {showCredit && creditData && (
                           <div className="bg-indigo-950/20 border border-indigo-900/30 rounded-xl p-5 animate-fade-in-up">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                 <div>
                                    <div className="text-[10px] uppercase text-indigo-300/60 font-bold mb-1">Límite</div>
                                    <div className="text-lg font-mono text-zinc-300">${Number(creditData.credit_limit ?? creditData.limit ?? 0).toFixed(2)}</div>
                                 </div>
                                 <div>
                                    <div className="text-[10px] uppercase text-indigo-300/60 font-bold mb-1">Balance Deuda</div>
                                    <div className="text-lg font-mono text-rose-400">${Number(creditData.balance ?? creditData.used ?? 0).toFixed(2)}</div>
                                 </div>
                                 <div className="col-span-2 mt-2 pt-3 border-t border-indigo-900/30 flex justify-between items-end">
                                    <div className="text-xs uppercase text-indigo-300/60 font-bold">Disponible</div>
                                    <div className="text-2xl font-black text-emerald-400 tracking-tight">${Number(creditData.available ?? 0).toFixed(2)}</div>
                                 </div>
                              </div>
                           </div>
                        )}
                              ...
                              <div className="max-h-48 overflow-y-auto p-2">
                                 <div className="space-y-1">
                                    {customerSales.map((s, i) => (
                                       <div key={i} className="flex justify-between items-center p-2 hover:bg-blue-900/20 rounded-lg transition-colors">
                                          <div>
                                             <div className="text-xs font-bold text-zinc-300 uppercase">{String(s.folio ?? s.id ?? '-')}</div>
                                             <div className="text-[10px] text-zinc-500 font-mono mt-0.5">{String(s.timestamp ?? s.created_at ?? '-').slice(0, 10)} / {String(s.payment_method ?? '-')}</div>
                                          </div>
                                          <div className="text-sm font-bold text-emerald-400 font-mono">${Number(s.total ?? 0).toFixed(2)}</div>
                                       </div>
                                    ))}
                                 </div>
                              </div>
               ...
               <div className="p-6 border-t border-zinc-800 bg-zinc-900/50 flex flex-col gap-3 shrink-0">
                  <button
                     onClick={() => void handleCreate()} disabled={busy || !name.trim()}
