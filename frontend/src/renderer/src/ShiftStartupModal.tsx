...
export default function ShiftStartupModal({
  ...
  const doOpenTurn = useCallback(async (cash: number): Promise<void> => {
    setBusy(true)
    setError('')
    try {
  ...
  const handleOpenSubmit = useCallback((e: FormEvent): void => {
    e.preventDefault()
  ...
  const handleCloseSubmit = useCallback(async (e: FormEvent): Promise<void> => {
    e.preventDefault()
  ...
  const handleNewShiftSubmit = useCallback((e: FormEvent): void => {
    e.preventDefault()
    const cash = parseFloat(initialCash)
    if (!Number.isFinite(cash) || cash < 0) {
      setError('Ingresa un monto valido')
  ...
  const inputCls = 'w-full rounded-lg border border-zinc-700 bg-zinc-950 py-2.5 px-3 text-sm font-semibold mb-3 focus:border-blue-500 focus:outline-none'
  const btnPrimary = 'w-full rounded-xl bg-blue-600 py-2.5 font-bold text-white hover:bg-blue-500 transition-colors disabled:opacity-40'
  const btnSecondary = 'w-full rounded-xl border border-zinc-700 bg-zinc-800 py-2.5 font-bold text-zinc-300 hover:bg-zinc-700 transition-colors'
  const btnDanger = 'w-full rounded-xl bg-rose-600/20 border border-rose-500/30 py-2.5 font-bold text-rose-400 hover:bg-rose-600/40 transition-colors'
  ...
  if (phase === 'no_shift') {
    return (
      <div className={backdrop}>
        <form onSubmit={handleOpenSubmit} onClick={(e) => e.stopPropagation()} className={card}>
          ...
          <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-1">Fondo inicial ($)</label>
          <input
            ref={inputRef}
            className={inputCls}
            type="number"
  ...
  if (phase === 'existing_shift' && existingShift) {
    ...
    const dateStr = openDate.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' })
        ...
        <div onClick={(e) => e.stopPropagation()} className={card}>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 mb-4 space-y-1">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-zinc-200">{dateStr} {timeStr}</span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          <div className="flex flex-col gap-2">
            <button onClick={handleContinue} className={btnPrimary}>Continuar turno</button>
            <button onClick={handleCloseAndOpen} className={btnSecondary}>Cerrar turno y abrir nuevo</button>
            <button onClick={onExit} className={btnDanger}>Cerrar programa</button>
          </div>
        </div>
      </div>
    )
  ...
  if (phase === 'closing_shift') {
    return (
      <div className={backdrop}>
        <form onSubmit={(e) => void handleCloseSubmit(e)} onClick={(e) => e.stopPropagation()} className={card}>
          <h2 className="text-lg font-bold text-rose-400 mb-4">Cerrar Turno</h2>
          <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-1">Efectivo en caja ($)</label>
          <input
            ref={inputRef}
            className={inputCls}
            type="number"
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => setPhase('existing_shift')} className={btnSecondary} disabled={busy}>
              Volver
            </button>
            <button type="submit" disabled={busy} className="flex-1 rounded-xl bg-rose-600 py-2.5 font-bold text-white hover:bg-rose-500 transition-colors disabled:opacity-40">
  ...
  if (phase === 'cut_summary' && cut) {
    const diffColor = cut.difference < 0 ? 'text-rose-400' : cut.difference > 0 ? 'text-amber-400' : 'text-emerald-400'
    return (
      <div className={backdrop}>
        <div onClick={(e) => e.stopPropagation()} className={card}>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 mb-4 space-y-2">
            ...
            {summary && (
              <div className="border-t border-zinc-800 pt-2 mt-2">
                <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-1">Resumen backend</p>
          ...
          <button onClick={handleAfterCut} className={btnPrimary}>Abrir nuevo turno</button>
        </div>
      </div>
    )
  }
  ...
  if (phase === 'opening_shift') {
    return (
      <div className={backdrop}>
        <form onSubmit={handleNewShiftSubmit} onClick={(e) => e.stopPropagation()} className={card}>
          <h2 className="text-lg font-bold text-blue-400 mb-4">Abrir Nuevo Turno</h2>
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 mb-4">
            <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold">Operador</p>
            <p className="text-sm font-semibold text-zinc-200 mt-0.5">{operator}</p>
