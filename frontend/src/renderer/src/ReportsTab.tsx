...
import { BarChart3, TrendingUp, DollarSign, Activity, PieChart, Calendar, Download, RefreshCw, ShoppingCart, CreditCard, Flame, Map, ArrowRight, FileText, Hash } from 'lucide-react'
...
export default function ReportsTab(): ReactElement {
  ...
  return (
    <div className="flex h-screen bg-[#09090b] font-sans text-slate-200 select-none overflow-y-auto w-full">
          ...
          <div className="flex items-center gap-3">
             <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-xl p-1">
                <input
                   ...
                   type="date" value={dateFrom} max={dateTo} onChange={(e) => setDateFrom(e.target.value)}
                />
                <span className="text-zinc-700 px-1">-</span>
                <input
                   className="bg-transparent py-1.5 px-3 text-sm font-medium focus:outline-none text-zinc-300 w-36"
                   type="date" value={dateTo} min={dateFrom} onChange={(e) => setDateTo(e.target.value)}
                />
             </div>
             <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-4 relative">
                 ...
                 <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Ingreso Bruto</span>
              </div>
              <div className="relative">
                 <p className="text-4xl font-black text-white font-mono tracking-tight">${totals.gross.toFixed(2)}</p>
           ...
           <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-4 relative">
                 ...
                 <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Ticket Promedio</span>
              </div>
              <div className="relative">
                 <p className="text-4xl font-black text-white font-mono tracking-tight">${totals.average.toFixed(2)}</p>
           ...
           <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6 relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-4 relative">
                 ...
                 <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">Ventas</span>
              </div>
              <div className="relative">
                 <p className="text-4xl font-black text-white font-mono tracking-tight">{totals.totalSales}</p>
                 <p className="text-sm font-medium text-zinc-500 mt-2 flex items-center gap-1">
                 ...
                 <div className="space-y-8">
                    <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-3xl p-6">
                       ...
                       <div className="space-y-4">
                          {Object.entries(totals.byMethod).length === 0 ? (
                             <p className="text-zinc-600 text-sm text-center py-4">Sin datos de métodos.</p>
                          ) : (
                             Object.entries(totals.byMethod).sort(([,a], [,b]) => b - a).map(([method, amount]) => {
                                   ...
                                   <div key={method}>
                                      <div className="flex justify-between items-end mb-1">
                                         <span className="text-sm font-bold text-zinc-300 capitalize flex items-center gap-2">
                                            {method === 'cash' ? <DollarSign className="w-4 h-4 text-emerald-500"/> :
                                             method === 'card' ? <CreditCard className="w-4 h-4 text-blue-500"/> :
                                             <RefreshCw className="w-4 h-4 text-purple-500"/>}
                                            {method}
                                         </span>
                                         <span className="text-sm font-mono text-zinc-400">${amount.toFixed(2)}</span>
                                      </div>
                    ...
                    <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-3xl p-6">
                       ...
                       <div className="space-y-3">
                          <button
                             onClick={exportSummaryCsv} disabled={busy || totals.totalSales === 0}
                          ...
                          >
                             <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                   ...
                                   <p className="text-[10px] text-zinc-500 uppercase tracking-wider">KPIs y Métodos (.CSV)</p>
                                </div>
                             </div>
                             <ArrowRight className="w-4 h-4 text-zinc-600 group-hover:text-white transition-colors" />
                          </button>
                          <button
                             onClick={exportTopProductsCsv} disabled={busy || totals.topProducts.length === 0}
                             className="w-full flex justify-between items-center bg-zinc-950 hover:bg-zinc-800 border border-zinc-800 rounded-xl p-4 transition-colors group disabled:opacity-50"
                          >
                             <div className="flex items-center gap-3">
                                 ...
                                 <div className="text-left">
                                   ...
                                   <p className="text-[10px] text-zinc-500 uppercase tracking-wider">SKUs más vendidos (.CSV)</p>
                                </div>
                             </div>
                             <ArrowRight className="w-4 h-4 text-zinc-600 group-hover:text-white transition-colors" />
                          </button>
                             ...
                             {(() => { const maxRevenue = Math.max(1, ...totals.topProducts.map(tp => tp.amount)); return totals.topProducts.map((p, idx) => {
                                ...
                                return (
                                   <tr key={p.sku} className="hover:bg-zinc-800/20 transition-colors">
                                      <td className="py-4 pr-4 text-zinc-600 font-bold text-xs">{idx + 1}</td>
                                      <td className="py-4 px-4 font-bold text-zinc-200 capitalize w-1/3 truncate max-w-[200px]" title={p.sku}>{p.sku}</td>
                                      <td className="py-4 px-4 text-right font-mono text-zinc-400">{p.qty}</td>
                                      <td className="py-4 pl-4 text-right font-mono font-bold text-emerald-400">${p.amount.toFixed(2)}</td>
              ...
              <div className="animate-fade-in-up bg-zinc-900/30 border border-zinc-800/50 rounded-3xl p-6">
                 <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                       ...
                       onClick={() => void loadDailySummary()} disabled={busy}
                    >
                 ...
                 <div className="overflow-x-auto rounded-xl border border-zinc-800/80 bg-zinc-950">
                    <table className="w-full text-left">
                       ...
                       <tbody className="divide-y divide-zinc-800/50">
                          {dailyData.length === 0 && (
                             <tr><td colSpan={4} className="py-12 text-center text-zinc-600">Presiona "Descargar del Servidor" para consultar históricos.</td></tr>
                          )}
                          {dailyData.map((d, i) => (
                             <tr key={i} className="hover:bg-zinc-900/50 transition-colors">
                                <td className="py-4 px-6 font-mono text-zinc-300">{String(d.date ?? d.fecha ?? '-')}</td>
                                <td className="py-4 px-6 font-mono text-zinc-400 text-right">{String(d.sales_count ?? d.ventas ?? 0)}</td>
                                <td className="py-4 px-6 font-mono font-bold text-emerald-400 text-right">${toNumber(d.total_amount ?? d.monto).toFixed(2)}</td>
                                <td className="py-4 px-6 font-mono text-blue-400 text-right">${toNumber(d.average_ticket ?? d.promedio).toFixed(2)}</td>
                             </tr>
                          ))}
                       </tbody>
                    </table>
           ...
           {subTab === 'ranking' && (
              <div className="animate-fade-in-up bg-zinc-900/30 border border-zinc-800/50 rounded-3xl p-6">
                 <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                       className="bg-zinc-900 border border-zinc-700 text-zinc-300 hover:text-white px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-50"
                       onClick={() => void loadRanking()} disabled={busy}
                    >
                       Descargar Global
                    </button>
                 </div>
                    ...
                    <table className="w-full text-left">
                       ...
                       <tbody className="divide-y divide-zinc-800/50">
                          {rankingData.length === 0 && (
                             <tr><td colSpan={4} className="py-12 text-center text-zinc-600">Presiona "Descargar Global" para obtener el histórico.</td></tr>
                          )}
                          {rankingData.map((d, i) => (
                             <tr key={i} className="hover:bg-zinc-900/50 transition-colors">
                                <td className="py-4 px-6 font-bold text-zinc-600 text-xs">{i + 1}</td>
                                <td className="py-4 px-6 font-bold text-zinc-200 capitalize">{String(d.product_name ?? d.sku ?? d.name ?? '-')}</td>
                                <td className="py-4 px-6 font-mono text-zinc-400 text-right">{String(d.quantity ?? d.qty ?? 0)}</td>
                                <td className="py-4 px-6 font-mono font-bold text-emerald-400 text-right">${toNumber(d.revenue ?? d.total).toFixed(2)}</td>
                             </tr>
                          ))}
                       </tbody>
                    </table>
           ...
           {subTab === 'heatmap' && (
              <div className="animate-fade-in-up bg-zinc-900/30 border border-zinc-800/50 rounded-3xl p-6">
                 <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                       className="bg-zinc-900 border border-zinc-700 text-zinc-300 hover:text-white px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-50"
                       onClick={() => void loadHeatmap()} disabled={busy}
                    >
                 ...
                 <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12 gap-3">
                    {(() => { const maxCount = Math.max(1, ...heatmapData.map((d) => toNumber(d.count ?? d.sales_count ?? 0))); return Array.from({ length: 24 }, (_, h) => {
                       ...
                       if (intensity > 0) bgClass = "bg-indigo-900/30 border-indigo-500/30 text-indigo-400";
                       if (intensity > 0.3) bgClass = "bg-indigo-800/50 border-indigo-400/50 text-indigo-300";
                       if (intensity > 0.6) bgClass = "bg-rose-900/50 border-rose-500/50 text-rose-300";
                       if (intensity > 0.8) bgClass = "bg-rose-600/80 border-rose-400 shadow-[0_0_15px_rgba(225,29,72,0.4)] text-white";
                       ...
                       return (
                          ...
                          >
                             <span className="absolute top-2 left-2 text-[10px] font-bold opacity-70 uppercase tracking-widest">{h}:00</span>
