      ...
      <div className="max-w-6xl mx-auto w-full p-6 md:p-8 space-y-8 pb-32">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-800 pb-6 relative">
          ...
          <div>
            <div className="flex items-center gap-3">
               ...
               <h1 className="text-3xl font-black text-white tracking-tight">Monitoreo y Control Satelital</h1>
            </div>
          ...
          <div className="flex items-center gap-3 relative z-10">
             <div className="bg-zinc-900 border border-zinc-800 px-4 py-2 rounded-xl text-xs font-bold font-mono tracking-widest text-zinc-400">
                PULSO DE RED: <span className={busy ? 'text-amber-500' : 'text-emerald-500'}>{busy ? 'TRABAJANDO' : 'ESTABLE'}</span>
             </div>
             <button
                onClick={() => { loadTurnStatus(); loadLiveSales(); loadNotifications() }} disabled={busy}
        ...
        {!canAdmin && (
          <div className="bg-amber-500/10 border border-amber-500/20 text-amber-500 px-5 py-4 rounded-2xl flex items-start gap-3 text-sm font-semibold max-w-2xl">
            ...
            <p>Atención: Cuentas con rol de solo lectura. Modificación remota, apertura de cajón y control de sucursales están deshabilitadas.</p>
          </div>
        )}
              ...
              <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6 relative overflow-hidden flex flex-col justify-center">
                 ...
                 {turnStatus ? (
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     <div>
                       <p className="text-xs uppercase tracking-wider text-zinc-500 font-bold mb-1">Estado</p>
                       <p className="text-lg font-black text-emerald-400">{String(turnStatus.status ?? turnStatus.state ?? '-')}</p>
                     </div>
                     <div>
                       <p className="text-xs uppercase tracking-wider text-zinc-500 font-bold mb-1">Operador</p>
                       <p className="text-lg font-bold text-white uppercase">{String(turnStatus.operator ?? turnStatus.opened_by ?? '-')}</p>
                     </div>
                     <div>
                       <p className="text-xs uppercase tracking-wider text-zinc-500 font-bold mb-1">Volumen</p>
                       <p className="text-lg font-mono text-zinc-300">{String(turnStatus.sales_count ?? 0)} tx</p>
                     </div>
                     <div>
                       <p className="text-xs uppercase tracking-wider text-zinc-500 font-bold mb-1">Arqueo Local</p>
                       <p className="text-lg font-mono font-bold text-blue-400">${toNumber(turnStatus.total_sales).toFixed(2)}</p>
                     </div>
                   </div>
                 ) : (
                 ...
                 <div className="overflow-x-auto rounded-xl border border-zinc-800/80 bg-zinc-950 max-h-[400px]">
                   <table className="w-full text-left">
                     ...
                     <tbody className="divide-y divide-zinc-800/50">
                       {liveSales.length === 0 && (
                         <tr><td colSpan={4} className="py-8 text-center text-zinc-600 font-mono text-xs">Esperando primera transmisión...</td></tr>
                       )}
                       {liveSales.map((sale, i) => (
                         <tr key={String(sale.id ?? i)} className="hover:bg-zinc-900/50 transition-colors group">
                           <td className="py-3 px-4 font-mono font-bold text-zinc-300 text-xs">{String(sale.folio ?? sale.id ?? '-')}</td>
                           <td className="py-3 px-4 text-xs font-mono text-zinc-500">{String(sale.timestamp ?? sale.created_at ?? '-').slice(11, 19)}</td>
                           <td className="py-3 px-4 text-xs font-bold uppercase text-zinc-400">{String(sale.payment_method ?? '-')}</td>
                           <td className="py-3 px-4 font-mono font-bold text-emerald-400 text-right text-xs group-hover:scale-110 transition-transform origin-right">${toNumber(sale.total).toFixed(2)}</td>
                         </tr>
                       ))}
                     </tbody>
                   </table>
              ...
              <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6">
                 ...
                 >
                   ...
                   <div className="text-left flex-1">
                      ...
                      <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Discutir pulso 24V al cajón</p>
                   </div>
                 </button>
              </div>
              ...
              <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6">
                 ...
                 <div className="space-y-3">
                   <input
                     ...
                     placeholder="SKU o Código de Barras" value={cpSku} onChange={(e) => setCpSku(e.target.value)}
                   />
                   <div className="flex gap-3">
                      <input
                        ...
                        placeholder="$ Nuevo" type="number" min={0} value={cpPrice} onChange={(e) => setCpPrice(e.target.value)}
                      />
                      <input
                        ...
                        placeholder="Justificación" value={cpReason} onChange={(e) => setCpReason(e.target.value)}
                      />
                   </div>
                   <button
                     ...
                     onClick={() => void handleChangePrice()} disabled={busy || !canAdmin}
                   >
                     Push Actualización
                   </button>
                 </div>
              ...
              <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-3xl p-6">
                 ...
                 <div className="space-y-3">
                   <input
                     ...
                     placeholder="Asunto" value={ntTitle} onChange={(e) => setNtTitle(e.target.value)}
                   />
                   <input
                     ...
                     placeholder="Cuerpo del mensaje..." value={ntBody} onChange={(e) => setNtBody(e.target.value)}
                   />
                   <div className="flex gap-3">
                      <select
                        ...
                        value={ntType} onChange={(e) => setNtType(e.target.value)}
                      >
                        <option value="info" className="text-blue-500">Informativo</option>
                        <option value="warning" className="text-amber-500">Advertencia</option>
                        <option value="alert" className="text-rose-500">Urgencia</option>
                      </select>
                      <button
                        ...
                        onClick={() => void handleSendNotification()} disabled={busy || !canAdmin}
                      >
                        Transmitir
                      </button>
                   </div>
                   ...
                   <div className="mt-6 pt-6 border-t border-zinc-800/80">
                      ...
                      <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                         {notifications.map((n, i) => (
                           <div key={String(n.id ?? i)} className="bg-zinc-950 rounded-lg p-3 border border-zinc-800/50 flex items-start gap-3">
                              <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${
                                String(n.notification_type ?? n.type) === 'alert' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]' :
                                String(n.notification_type ?? n.type) === 'warning' ? 'bg-amber-500' : 'bg-blue-500'
                              }`}></div>
                              <div>
                                 <p className="text-xs font-bold text-zinc-300">{String(n.title ?? '-')}</p>
                                 <p className="text-[10px] text-zinc-500 font-mono mt-0.5">{String(n.created_at ?? n.timestamp ?? '-').slice(0, 19)}</p>
                              </div>
                           </div>
                         ))}
                      </div>
