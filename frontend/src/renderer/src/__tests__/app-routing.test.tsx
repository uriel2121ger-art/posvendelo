...
// Mock posApi: usar importOriginal y solo reemplazar lo que el test necesita (evita faltar exports).
vi.mock('../posApi', async (importOriginal) => {
...
describe('App Routing', () => {
  ...
  it('auto-configura URL y redirige a /login en desktop sin URL configurada', async () => {
    localStorage.removeItem('pos.baseUrl')
    window.location.hash = '#/'
    render(<App />)

    await waitFor(() => {
      expect(window.location.hash).toBe('#/login')
      expect(localStorage.getItem('pos.baseUrl')).toBeTruthy()
    })
  })
