      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="px-8 py-6 border-b border-zinc-900 bg-zinc-950 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
               onClick={() => { setSku(''); setIsDrawerOpen(true); setTimeout(() => skuInputRef.current?.focus(), 100); }} disabled={busy}
        ...
        <div className="shrink-0 flex flex-col gap-2 px-8">
               ...
               <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 text-xs">
                 {alerts.map((a, i) => (
                   <div key={i} className="flex justify-between items-center rounded-lg bg-zinc-900 px-3 py-2 border border-amber-900/30">
                     <span className="text-zinc-300 truncate pr-2 font-medium">{String((a as any).sku ?? (a as any).product_name ?? `#${i}`)}</span>
                     <span className="text-rose-400 font-mono font-bold bg-rose-950/50 px-2 py-0.5 rounded">{String((a as any).stock ?? (a as any).current_stock ?? '?')}</span>
                   </div>
                 ))}
               </div>
             </div>
           ...
           {showMovements && (
             <div className="max-h-64 overflow-hidden flex flex-col rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 relative">
               ...
               {movements.length === 0 ? (
                  <p className="text-center text-zinc-500 py-4 text-sm">No hay movimientos recientes.</p>
               ) : (
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                           {movements.map((m, i) => (
                              <tr key={i} className="hover:bg-zinc-800/30">
                                 <td className="px-3 py-2 text-zinc-400 font-mono">{String((m as any).timestamp ?? (m as any).created_at ?? '-').slice(0, 16).replace('T', ' ')}</td>
                                 <td className="px-3 py-2 text-zinc-200 truncate max-w-[150px]">{String((m as any).product_name ?? (m as any).sku ?? (m as any).product_id ?? '-')}</td>
                                 <td className="px-3 py-2">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${String((m as any).movement_type ?? (m as any).type) === 'IN' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
                                       {String((m as any).movement_type ?? (m as any).type ?? '-')}
                                    </span>
                                 </td>
                                 <td className="px-3 py-2 font-mono font-bold text-zinc-300">{String((m as any).quantity ?? '-')}</td>
                                 <td className="px-3 py-2 text-zinc-500 truncate max-w-[200px]">{String((m as any).reason ?? '-')}</td>
                              </tr>
                           ))}
                        </tbody>
                     </table>
                    ...
                    {paginated.length === 0 ? (
                       <tr>
                          <td colSpan={4} className="px-6 py-16 text-center text-zinc-500">
                             ...
                             <p className="text-sm mt-1">{query.trim() ? 'Intenta con otra búsqueda.' : 'Haz clic en "Sincronizar".'}</p>
                          </td>
                       </tr>
                    ) : (
                       paginated.map(r => (
                          <tr
                             key={r.id}
                             onClick={() => { setSku(r.sku); setIsDrawerOpen(true); }}
                             className="group hover:bg-zinc-800/40 cursor-pointer transition-colors"
                          >
                             <td className="px-6 py-4 font-mono text-sm text-zinc-400 group-hover:text-indigo-400 transition-colors">{r.sku}</td>
                             <td className="px-6 py-4 font-medium text-zinc-200">{r.name}</td>
                             <td className="px-6 py-4 text-right">
                                <span className={`inline-flex items-center justify-center min-w-[3rem] px-3 py-1.5 rounded-lg text-sm font-bold shadow-inner ${r.stock <= 5 ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-zinc-950 text-emerald-400 border border-zinc-800'}`}>
                                   {r.stock}
                                </span>
                             </td>
                             <td className="px-6 py-4 text-right">
         ...
         >
            ...
            >
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  {/* SKU Input & Validator */}
                  <div>
                     <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">ESCANEAR PRODUCTO A MOVER (SKU)</label>
                     ...
                     {(() => {
                        ...
                        if (!match) return <div className="mt-2 text-xs text-rose-400">Producto no reconocido.</div>;
                        return (
                           <div className="mt-3 bg-indigo-950/20 border border-indigo-900/30 rounded-xl p-4">
                              <div className="text-sm font-semibold text-zinc-200 line-clamp-2">{match.name}</div>
                              <div className="mt-2 text-xs font-mono text-indigo-300">Stock Actual: <span className="font-bold text-lg ml-1 text-white">{match.stock}</span></div>
                           </div>
                        );
                     })()}
                  </div>
                  ...
                  {/* Operation Type */}
                  <div>
                     <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-3">TIPO DE OPERACIÓN</label>
                  ...
                  {/* Quantity */}
                  <div>
                     <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">CANTIDAD A APLICAR</label>
                     <div className="flex bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-inner">
                        <button onClick={() => setMovementQty(q => String(Math.max(1, Number(q) - 1)))} className="px-5 text-zinc-400 hover:bg-zinc-800 hover:text-white font-bold text-lg">-</button>
                        <input
                           type="number" value={movementQty} onChange={e => setMovementQty(e.target.value)} min={1}
                           className="w-full bg-transparent py-4 text-center text-xl font-black text-white focus:outline-none"
                        />
                        <button onClick={() => setMovementQty(q => String(Number(q) + 1))} className="px-5 text-zinc-400 hover:bg-zinc-800 hover:text-white font-bold text-lg">+</button>
                     </div>
                  </div>
               </div>
               ...
               <div className="p-6 border-t border-zinc-800 bg-zinc-900/80">
                  <button
                     onClick={() => void handleAdjustStock()} disabled={busy || !sku.trim()}
