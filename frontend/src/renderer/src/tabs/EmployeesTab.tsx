...
export default function EmployeesTab(): ReactElement {
  ...
  async function handleDelete(): Promise<void> {
    ...
    if (!target) return
    if (
      !(await confirm(`¿Eliminar empleado "${target.name}"?`, {
        variant: 'danger',
  ...
  return (
    <div className="flex h-full bg-[#09090b] font-sans text-slate-200 select-none overflow-hidden relative">
      ...
      {message &&
        message !== 'Empleados: gestiona tu equipo de trabajo.' &&
        !message.startsWith('Empleados cargados') && (
      ...
      {/* Detail Slide Drawer */}
      <div
        className={`fixed inset-0 z-[100] flex justify-end transition-opacity duration-300 ${isDrawerOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
        {/* Backdrop */}
        <div
