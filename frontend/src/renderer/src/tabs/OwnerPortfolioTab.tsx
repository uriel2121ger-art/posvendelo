        ...
        <div className="max-w-6xl mx-auto w-full p-4 lg:p-6 space-y-6 pb-24">
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-zinc-500 truncate">
                  {data?.tenantName ?? 'Tenant sin resolver'} {data?.tenantSlug ? `• ${data.tenantSlug}` : ''}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 lg:p-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-xs text-zinc-500 uppercase tracking-wider">Días vigencia</div>
                ...
                {(commercial?.events ?? []).map((event, index) => (
                  <div key={`${String(event.id ?? index)}`} className="px-4 py-3 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-semibold text-white">{String(event.event_type ?? 'evento')}</div>
                      <div className="text-[11px] font-mono text-zinc-500">
                        {String(event.created_at ?? '-').slice(0, 19).replace('T', ' ')}
                      </div>
                    </div>
                    <div className="mt-1 text-zinc-400">Actor: {String(event.actor ?? 'sistema')}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              ...
              ['Ventas hoy', `$${(data?.salesTodayTotal ?? 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}`]
            ].map(([label, value]) => (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4 lg:p-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-xs text-zinc-500 uppercase tracking-wider">Backups vencidos</div>
                  <div className="mt-1 font-mono text-cyan-300">
                    {String(healthSummary?.summary?.stale_backups ?? 0)}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-zinc-500 uppercase tracking-wider">Deriva versión</div>
                  <div className="mt-1 font-mono text-cyan-300">
              ...
              <div className="mt-4 text-xs text-zinc-500">
                POS esperado: {String(healthSummary?.summary?.expected_pos_version ?? 'sin referencia')}
              </div>
            </div>
                ...
                {(auditFeed?.audit ?? []).map((entry, index) => (
                  <div key={`${String(entry.id ?? index)}`} className="px-4 py-3 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      <div className="font-semibold text-white">{String(entry.action ?? 'acción')}</div>
                      <div className="text-[11px] font-mono text-zinc-500">
                        {String(entry.created_at ?? '-').slice(0, 19).replace('T', ' ')}
                      </div>
                    </div>
                    <div className="mt-1 text-zinc-300">
                      {String(entry.branch_name ?? 'Sucursal desconocida')} • {String(entry.actor ?? 'sistema')}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-zinc-800/50 text-sm">
                  ...
                  {branches.map((branch, index) => {
                    ...
                    return (
                      <tr key={String(branch.id ?? index)} className="hover:bg-zinc-800/30 transition-colors">
                        <td className="px-4 py-2">
                          <div className="font-semibold text-white">{String(branch.branch_name ?? branch.branch_slug ?? '-')}</div>
                          <div className="text-xs text-zinc-500">{String(branch.branch_slug ?? '')}</div>
                        </td>
                        <td className={`px-4 py-2 font-semibold ${online ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {online ? 'Online' : 'Offline'}
                        </td>
                        <td className="px-4 py-2 text-zinc-300">{String(branch.release_channel ?? '-')}</td>
                        <td className="px-4 py-2 text-zinc-300">{String(branch.pos_version ?? '-')}</td>
                        <td className="px-4 py-2 text-right font-mono text-cyan-300">
                          ${Number(branch.sales_today ?? 0).toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-zinc-400">
                          {branch.last_backup ? String(branch.last_backup).slice(0, 19).replace('T', ' ') : 'Sin respaldo'}
                        </td>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
            ...
            <div className="divide-y divide-zinc-800/50">
              {alerts.length === 0 && (
                <div className="px-4 py-8 text-sm text-zinc-500">Sin alertas activas para este tenant.</div>
              )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
              ...
              <div className="divide-y divide-zinc-800/50 max-h-[28rem] overflow-auto">
                {events.length === 0 && (
                  <div className="px-4 py-8 text-sm text-zinc-500">Sin eventos recientes para este tenant.</div>
                )}
                {events.map((event, index) => (
                  <div key={`${String(event.event_type)}-${index}`} className="px-4 py-3 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      <div className="font-semibold text-white">{String(event.event_type ?? 'evento')}</div>
                      <div className="text-[11px] font-mono text-zinc-500">
                        {String(event.occurred_at ?? '-').slice(0, 19).replace('T', ' ')}
                      </div>
                    </div>
                    <div className="mt-1 text-zinc-300">{String(event.message ?? '')}</div>
                    <div className="mt-1 text-[11px] text-zinc-500">
                      {String(event.branch_name ?? 'Sucursal desconocida')} • {String(event.source ?? 'sistema')}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="divide-y divide-zinc-800/50 max-h-[28rem] overflow-auto">
                ...
                {timeline.map((item, index) => (
                  <div key={`${String(item.heartbeat_id)}-${index}`} className="px-4 py-3 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      <div className="font-semibold text-white">{String(item.status ?? 'unknown')}</div>
                      <div className="text-[11px] font-mono text-zinc-500">
                        {String(item.received_at ?? '-').slice(0, 19).replace('T', ' ')}
                      </div>
                    </div>
