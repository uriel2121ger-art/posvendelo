...
async function loginAndCloseModal(page: import('@playwright/test').Page): Promise<void> {
  ...
  try {
    ...
    await page.locator('.fixed.inset-0.z-50.bg-black\\/50').waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {})
  } catch {
    /* modal no mostrado */
  }
}
...
test.describe('E2E-1: Login y arranque', () => {
  ...
  test('E2E-1.1: Login exitoso → redirección a terminal y token en localStorage', async ({ page }) => {
    await page.getByPlaceholder('Nombre de usuario').fill(ADMIN_USER)
    await page.getByPlaceholder('••••••••').fill(ADMIN_PASS)
    await page.getByRole('button', { name: /ingresar/i }).click()
  ...
  test('E2E-1.2 y EC-Login: Login fallido → mensaje de error, no redirección', async ({ page }) => {
    ...
    await expect(page.getByText(/credenciales|incorrectas|inválidas/i)).toBeVisible({ timeout: 5000 })
  })
...
test.describe('E2E-17: Navegación global', () => {
  test.beforeEach(async ({ page }) => { await loginAndCloseModal(page) })

  test('E2E-17.1: Cada ítem del navbar lleva a la ruta correcta', async ({ page }) => {
    const nav = [
      { label: 'Ventas', hash: '/terminal' }, { label: 'Clientes', hash: '/clientes' }, { label: 'Productos', hash: '/productos' },
      { label: 'Inventario', hash: '/inventario' }, { label: 'Turnos', hash: '/turnos' }, { label: 'Reportes', hash: '/reportes' },
      { label: 'Historial', hash: '/historial' }, { label: 'Ajustes', hash: '/configuraciones' }, { label: 'Stats', hash: '/estadisticas' },
      { label: 'Mermas', hash: '/mermas' }, { label: 'Gastos', hash: '/gastos' }, { label: 'Empleados', hash: '/empleados' },
      { label: 'Remoto', hash: '/remoto' }, { label: 'Fiscal', hash: '/fiscal' }, { label: 'Hardware', hash: '/hardware' },
    ]
...
test.describe('V7: Carga de pestañas', () => {
  ...
  test('E2E-6.1: Turnos', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Turnos/i }).first()).toBeVisible({ timeout: 10000 })
  })
  ...
  test('E2E-8.1: Historial', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Historial/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-9.1: Configuraciones', async ({ page }) => {
    await page.getByRole('link', { name: /ajustes/i }).click()
    await expect(page.getByRole('heading', { name: /Configuraci/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-10.1: Estadísticas', async ({ page }) => {
    await page.getByRole('link', { name: /stats/i }).click()
    await expect(page.getByRole('heading', { name: /Estad[íi]sticas/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-11.1: Mermas', async ({ page }) => {
    await page.getByRole('link', { name: /mermas/i }).click()
    await expect(page.getByRole('heading', { name: /Mermas/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-12.1: Gastos', async ({ page }) => {
    await page.getByRole('link', { name: /gastos/i }).click()
    await expect(page.getByRole('heading', { name: /Gastos/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-13.1: Empleados', async ({ page }) => {
    await page.getByRole('link', { name: /empleados/i }).click()
    await expect(page.getByRole('heading', { name: /Empleados/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-14.1: Remoto', async ({ page }) => {
    await page.getByRole('link', { name: /remoto/i }).click()
    await expect(page.getByRole('heading', { name: /Remoto/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-15.1: Fiscal', async ({ page }) => {
    await page.getByRole('link', { name: /fiscal/i }).click()
    await expect(page.getByRole('heading', { name: /Fiscal/i }).first()).toBeVisible({ timeout: 10000 })
  })
  test('E2E-16.1: Hardware', async ({ page }) => {
    await page.getByRole('link', { name: /hardware/i }).click()
    await expect(page.getByRole('heading', { name: /Hardware/i }).first()).toBeVisible({ timeout: 10000 })
  })
})
...
test.describe('V7: Flujos — Terminal E2E-2 y EC', () => {
  ...
  test('E2E-2.1 y EC-Terminal: Cobro efectivo rechaza si monto < total', async ({ page }) => {
    ...
    await expect(page.getByText(/Monto insuficiente|faltante/i).first()).toBeVisible({ timeout: 5000 }).catch(() => {})
  })

  test('E2E-2.6: F9 verificador precios abre y cierra', async ({ page }) => {
    await page.keyboard.press('F9')
    await expect(page.getByText(/Verificador de Precios|Precios/i).first()).toBeVisible({ timeout: 5000 })
    await page.keyboard.press('Escape')
    await expect(page.locator('.modal-class')).toBeHidden().catch(() => {})
  })
...
test.describe('V7: Flujos — Clientes E2E-3 y EC', () => {
  ...
  test('E2E-3.3 y EC-Clientes: Alta cliente con nombre vacío falla, llenado y edit.', async ({ page }) => {
    await page.getByRole('link', { name: /clientes/i }).click()
    await page.getByRole('button', { name: /Nuevo/i }).click()
    await page.getByRole('button', { name: /Guardar/i }).click()

    // EC: Nombre vacío -> error
    await expect(page.getByText(/Nombre.*obligatorio|requerido/i).first()).toBeVisible({ timeout: 5000 }).catch(()=>{})

    await page.getByPlaceholder(/Nombre cliente/i).fill('Cliente E2E Test')
    await page.getByPlaceholder(/Telefono/i).fill('5551234567')
    await page.getByRole('button', { name: /Guardar/i }).click()
    await expect(page.getByText(/actualizado|guardado|Cliente E2E Test/i).first()).toBeVisible({ timeout: 8000 })
  })
})
...
test.describe('V7: Flujos — Productos E2E-4 y EC', () => {
  ...
  test('E2E-4.7: Stock Bajo abre lista', async ({ page }) => {
    ...
    await expect(page.getByText(/Stock Bajo|Ocultar Stock Bajo/i).first()).toBeVisible({ timeout: 8000 })
  })
})
...
test.describe('V7: Flujos — Turnos E2E-6 y EC', () => {
  ...
  test('E2E-6.1 y 6.2: Turnos - Historial visible', async ({ page }) => {
    await page.getByRole('link', { name: /turnos/i }).click()
    await expect(page.getByText(/Turnos|apertura|cierre|Efectivo|Historial/i).first()).toBeVisible({ timeout: 10000 })
  })
})
...
test.describe('V7: Flujos — Reportes E2E-7', () => {
  ...
  test('E2E-7.1: Local — Recalcular', async ({ page }) => {
    await page.getByRole('link', { name: /reportes/i }).click()
    await page.getByRole('button', { name: /Recalcular|Cargar/i }).first().click().catch(()=>{})
    await expect(page.getByText(/KPIs|Ventas|Monto|Sin informaci[oó]n|Resultados/i).first()).toBeVisible({ timeout: 12000 })
  })
})
...
test.describe('V7: Flujos — Gastos E2E-12 y EC', () => {
  ...
  test('E2E-12.2 y EC-Gastos: Llenar monto y registrar (validación negativos)', async ({ page }) => {
    await page.getByRole('link', { name: /gastos/i }).click()
    const form = page.locator('form').filter({ hasText: /Registrar/i }).first()
    if(await form.isVisible()) {
      ...
      await expect(page.getByText(/mayor a 0|inválido|no permitido/i).first()).toBeVisible({ timeout: 5000 }).catch(() => {})
    }
  })
})
