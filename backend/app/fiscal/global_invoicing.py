...
class GlobalInvoicingService:
    ...
    def get_pending_sales_summary(
        ...
        sql = """
            ...
            FROM sales s
            WHERE s.serie = 'B'
            AND CAST(s.timestamp AS DATE) BETWEEN %s AND %s
        ...
        try:
            ...
            if result.get('success'):
                ...
                for sale in sales:
                    ...
                    if cfdi_id:
                        # CRITICAL FIX: Check if relation already exists before inserting
                        # This avoids ON CONFLICT errors when UNIQUE constraint doesn't exist
                        existing = self.core.db.execute_query(
                            "SELECT id FROM sale_cfdi_relation WHERE sale_id = %s AND cfdi_id = %s",
                            (sale['id'], cfdi_id)
                        )
                        if not existing:
                            try:
                                self.core.db.execute_write(
                                    "INSERT INTO sale_cfdi_relation (sale_id, cfdi_id, is_global) VALUES (%s, %s, 1)",
                                    (sale['id'], cfdi_id)
                                )
                            except Exception as e:
                                # If insert fails (e.g., constraint violation), log but don't fail the whole operation
    ...
    def _generate_cfdi_data(
        ...
        try:
            ...
            if len(aggregated['items']) <= 50:
                # Detailed by product
                for idx, item in enumerate(aggregated['items']):
                    ...
                    # CRITICAL FIX: If normalized key is not a valid SAT code, use default
                    # Facturapi may reject keys that don't exist in their catalog
                    # Use '01010101' (No existe en catálogo) as fallback for invalid keys
                    # This is a valid generic code that Facturapi accepts
                    if sat_code_normalized != '01010101' and len(sat_code_normalized) == 8:
                        # Try to use the normalized key, but we'll fallback if Facturapi rejects it
                        product_key = sat_code_normalized
                    else:
                        # Use default generic code
            ...
            if facturapi_enabled and facturapi_api_key:
                try:
                    ...
                    if facturapi:
                        ...
                        mes = _date_parsed.strftime('%m')
                        año = _date_parsed.strftime('%Y')
                        
                        # CRITICAL FIX: Get valid postal code from fiscal config
                        ...
                        if not postal_code or postal_code == '00000':
                            # Use a valid default postal code (01000 = Mexico City)
                            postal_code = '01000'
                    ...
                    if "product_key" in error_str.lower() or "couldn't find" in error_str.lower():
                        ...
                        try:
                            ...
                            if result and isinstance(result, dict):
                                ...
                                """, (
                                    uuid, 'G', str(folio), datetime.now().isoformat(),
                                    aggregated['total'], aggregated['subtotal'], aggregated['tax'],
                                    self.RFC_PUBLICO_GENERAL, 'PUBLICO EN GENERAL', 'S01', '01', 'PUE',
                                    'timbrado', facturapi_id, 'synced', datetime.now().isoformat()
                                ))
                                
                                logger.info(f"✅ Global invoice stamped (retry): UUID={uuid}, Folio=G{folio}")
                                ...
                                return {
                                    ...
                                    'facturapi_id': facturapi_id,
                                    'folio': f'G{folio}',
                                    'message': f"✅ Factura global timbrada (con código genérico): {uuid}"
                                }
                        except Exception as retry_error:
                            logger.error(f"Facturapi retry also failed: {retry_error}")
