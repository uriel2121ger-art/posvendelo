...
Uso:
    from app.fiscal.error_translator import translate_sat_error, get_solution
...
SAT_ERROR_TRANSLATIONS = {
    ...
    "LCO": {
        ...
        "user_message": "⛔ El RFC está en la Lista Negra del SAT (Artículo 69-B)",
...
def format_error_for_ui(error_code: str = None, 
    ...
    emoji = emojis.get(translation['category'], '⚠️')
    
    return f"""
{emoji} **{translation['user_message']}**

**Código:** {translation['code']}
**Categoría:** {translation['category']}

**Solución:**
{translation['solution']}

---
_Error técnico: {translation['technical']}_
""".strip()

class SATErrorTranslator:
    """
    Wrapper class for SAT error translation functions.

    Provides an object-oriented interface for translating SAT error codes
    and messages into user-friendly explanations.
    """

    def translate(self, error_message: str, error_code: str = None) -> str:
        """
        Translate a SAT error message to a user-friendly message.

        Args:
            error_message: The original error message from SAT/PAC
            error_code: Optional error code

        Returns:
            Translated user-friendly message
        """
        result = translate_sat_error(error_code, error_message)
        return result.get('user_message', error_message)

    def get_full_translation(self, error_message: str, error_code: str = None) -> Dict[str, Any]:
        """
        Get full translation details including solution and technical info.
        ...
        Returns:
            Dictionary with user_message, solution, technical details
        """
        return translate_sat_error(error_code, error_message)

    def format_for_ui(self, error_message: str, error_code: str = None) -> str:
        """
        ...
        """
        return format_error_for_ui(error_code, error_message)


if __name__ == "__main__":
    print("SAT Error Translator Test\n")

    # Test con códigos conocidos
    test_cases = [
        ...
        (None, "El timeout de conexión fue excedido"),
