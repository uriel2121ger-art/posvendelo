...
class ClimateShield:
    ...
    async def evaluate_degradation_risk(self, climate: Dict, 
        ...
        return {
            ...
            'factors': risks,
            'recommendation': await self._get_recommendation(risk_level),
            'scientific_basis': await self._get_scientific_basis(temp, humidity)
        }
    
    async def _get_recommendation(self, risk_level: str) -> str:
    ...
    async def _get_scientific_basis(self, temp: float, humidity: float) -> str:
    ...
    async def generate_shrinkage_justification(self, merma_data: Dict) -> Dict[str, Any]:
        ...
        justification = {
            ...
            # Hash de integridad
            'integrity_hash': await self._generate_hash({
    ...
    async def _generate_hash(self, data: Dict) -> str:
    ...
    async def attach_to_merma(self, merma_id: int) -> Dict:
        ...
        merma = list(await self.core.db.execute_query(
        ...
        justification = await self.generate_shrinkage_justification(merma_data)
...
async def enhance_materiality_with_climate(core, merma_id: int) -> Dict:
    """Añade justificación climática a una merma."""
    shield = ClimateShield(core)
    return await shield.attach_to_merma(merma_id)
