...
class WealthDashboard:
    ...
    async def get_real_wealth(self, year: int = None, 
        ...
        return {
            ...
            'recomendaciones': await self._get_recommendations(disponible, utilidad_neta)
        }
    
    async def _get_total_income(self, date_filter: str, 
        ...
        if period_type == 'month':
            sql = """
                ...
                FROM sales
                WHERE TO_CHAR(timestamp::timestamp, 'YYYY-MM') = %s
            ...
            sql = """
                SELECT 
                    ...
                    COUNT(*) as transactions
                FROM sales
                WHERE EXTRACT(YEAR FROM timestamp::timestamp) = %s
                AND status = 'completed'
                GROUP BY serie
            """
        
        result = list(await self.core.db.execute_query(sql, (date_filter,)))
    ...
    async def _get_operating_expenses(self, date_filter: str,
                                 period_type: str) -> Dict[str, Any]:
    ...
    async def _calculate_taxes(self, ingresos: Dict, 
                         period_type: str) -> Dict[str, Any]:
    ...
    async def _get_extractions(self, date_filter: str,
                         period_type: str) -> Dict[str, Any]:
    ...
    async def _get_recommendations(self, disponible: float,
