...
class DiscrepancyMonitor:
    ...
    def __init__(self, core):
        ...
        import asyncio
        try:
            asyncio.get_running_loop().create_task(self._setup_table())
        except RuntimeError:
            pass
    ...
    async def register_expense(self, amount: float, category: str,
    ...
    async def get_discrepancy_analysis(self, year: int = None, 
        ...
        ingresos = list(await self.core.db.execute_query(ingresos_sql, (date_filter,)))
        ...
        gastos = list(await self.core.db.execute_query(gastos_sql, (date_filter,)))
        ...
        try:
            if period_type == 'month':
                ...
                """
            extracciones = list(await self.core.db.execute_query(ext_sql, (date_filter,)))
        ...
        return {
            ...
            'estado': estado,
            'semaforo': semaforo,
            'recomendaciones': await self._get_recommendations(
                estado, discrepancia, total_extracciones
            )
        }
    
    async def _get_recommendations(self, estado: str, discrepancia: Decimal,
    ...
    async def get_monthly_trend(self, year: int = None) -> List[Dict[str, Any]]:
        ...
        for m in range(1, 13):
            ...
            analysis = await self.get_discrepancy_analysis(year, m)
            months.append({
                'month': m,
    ...
    async def suggest_extraction_amount(self) -> Dict[str, Any]:
        ...
        analysis = await self.get_discrepancy_analysis()
    ...
    async def get_expense_breakdown(self, year: int = None, 
                               month: int = None) -> Dict[str, Any]:
        ...
        if month:
            ...
            """
            result = list(await self.core.db.execute_query(sql, (f"{year}-{month:02d}",)))
        else:
            sql = """
                SELECT category, payment_method,
                       COALESCE(SUM(amount), 0) as total,
                ...
                GROUP BY category, payment_method
            """
            result = list(await self.core.db.execute_query(sql, (str(year),)))
