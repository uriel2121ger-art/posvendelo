...
class CashExtractionEngine:
    ...
    def __init__(self, core):
        ...
        import asyncio
        try:
            asyncio.get_running_loop().create_task(self._setup_tables())
        except RuntimeError:
            pass
    ...
    async def add_related_person(self, name: str, parentesco: str,
    ...
    async def get_serie_b_balance(self) -> Dict[str, Any]:
        ...
        extracted = list(await self.core.db.execute_query(
    ...
    async def create_extraction(self, amount: float, document_type: str,
        ...
        if related_person_id:
            p = list(await self.core.db.execute_query(
    ...
    async def generate_contract_text(self, extraction_id: int) -> str:
        ...
        ext = list(await self.core.db.execute_query(
        ...
        if e['document_type'] == 'DONACION':
            contract = await self._generate_donation_contract(e, config, fiscal, fecha)
        else:
            contract = await self._generate_loan_contract(e, config, fiscal, fecha)
        
        return contract
    
    async def _generate_donation_contract(self, e: Dict, config: Dict, 
...
El DONANTE transmite de forma gratuita al DONATARIO la cantidad de:
        ...
        ({await self._num_to_words(e['amount'])} PESOS 00/100 M.N.)
...
"""
    ...
    async def _generate_loan_contract(self, e: Dict, config: Dict,
                                fiscal: Dict, fecha: datetime) -> str:
...
"""
    ...
    async def _num_to_words(self, num) -> str:
