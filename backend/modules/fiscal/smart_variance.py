...
class SmartVarianceEngine:
    ...
    async def _natural_round(self, amount: float) -> float:
        """
    ...
    async def generate_batch_variance(self, items: List[Dict], 
        ...
        for i, item in enumerate(items[:-1]):
            ...
            adjusted = await self._natural_round(item.get('amount', 0) * individual_factor)
    ...
    async def get_seasonal_factor(self, date: datetime = None) -> float:
        """
