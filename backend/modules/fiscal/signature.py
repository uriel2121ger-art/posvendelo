...
def _get_secure_xml_parser():
    """
    SECURITY: Create a secure lxml parser that prevents XXE attacks.
...
class CFDISignature:
    ...
    def sign_xml(self, xml_string: str) -> Dict[str, str]:
        ...
        # SECURITY: Parse XML with secure parser (prevents XXE attacks)
...
def sign_cfdi_xml(xml_string: str, fiscal_config: Optional[Dict[str, Any]]) -> str:
    ...
    Raises:
        ValueError: If fiscal_config is None/empty or required keys are missing
    """
    # FIX I5/I7: Validar fiscal_config y sus claves requeridas
    if not fiscal_config:
        raise ValueError("fiscal_config is required and cannot be None or empty")

    required_keys = ['csd_cert_path', 'csd_key_path', 'csd_key_password']
    missing = [k for k in required_keys if not fiscal_config.get(k)]
    if missing:
        raise ValueError(f"fiscal_config missing required keys: {', '.join(missing)}")
