...
class SelfConsumptionEngine:
    ...
    def get_monthly_summary(self, year: int = None, 
        ...
        sql = """
            ...
            WHERE EXTRACT(YEAR FROM created_at::timestamp)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
            AND LPAD(EXTRACT(MONTH FROM created_at::timestamp)::TEXT, 2, '0') = %s  -- FIX 2026-02-01: PostgreSQL
            GROUP BY category
        """
    ...
    def generate_monthly_voucher(self, year: int = None, 
        ...
        # Generar documento
        from src.services.fiscal.legal_documents import LegalDocumentGenerator
    ...
    def get_pending_voucher_months(self) -> List[str]:
        ...
        sql = """
            SELECT DISTINCT TO_CHAR(created_at::timestamp, 'YYYY-MM') as period  -- FIX 2026-02-01: PostgreSQL
            FROM self_consumption
