...
class RESICOMonitor:
    ...
    def _get_ventas_serie_a(self, year: int) -> Decimal:
        ...
        sql = """
            ...
            FROM sales 
            WHERE serie = 'A' 
            AND EXTRACT(YEAR FROM timestamp)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
    ...
    def get_monthly_breakdown(self, year: int = None) -> List[Dict[str, Any]]:
        ...
        sql = """
            SELECT
                EXTRACT(MONTH FROM timestamp)::TEXT as mes,  -- FIX 2026-02-01: PostgreSQL
            ...
            AND EXTRACT(YEAR FROM timestamp)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
            AND status = 'completed'
            GROUP BY EXTRACT(MONTH FROM timestamp)::TEXT  -- FIX 2026-02-01: PostgreSQL
            ORDER BY mes
        """
        ...
        for row in result:
            ...
            months.append({
                ...
                'acumulado': acumulado,
                # CRITICAL FIX: Safe division - avoid division by zero
                'porcentaje_limite': round((acumulado / float(self.LIMITE_ANUAL)) * 100, 2)
                                     if self.LIMITE_ANUAL and float(self.LIMITE_ANUAL) > 0 else 0
            })
