...
Uso:
    from src.services.fiscal.timezone_handler import get_cfdi_timestamp, get_timezone_for_cp
...
# Intentar importar pytz
try:
    import pytz
    HAS_PYTZ = True
except ImportError:
    HAS_PYTZ = False
    logger.warning("pytz no instalado. Ejecutar: pip install pytz")
...
TIMEZONE_ZONES = {
    ...
    # Zona Pacífico (Sonora, Sinaloa, Nayarit, BCS) - UTC-7
    'America/Mazatlan': [
        (23000, 23999),  # Baja California Sur
        (80000, 85999),  # Sonora, Sinaloa
...
def get_cfdi_timestamp(codigo_postal: str = None, 
    ...
    Args:
        ...
        dt: Datetime específico (default: ahora)
    ...
    if HAS_PYTZ and codigo_postal:
        tz_name = get_timezone_for_cp(codigo_postal)
        tz = pytz.timezone(tz_name)
        
        # Si dt es naive (sin timezone), localizarlo
        if dt.tzinfo is None:
            # Asumir que dt está en UTC si viene del servidor
        ...
        # Sin pytz o sin CP, usar hora local del sistema
        local_dt = dt
...
def validate_cfdi_timestamp(timestamp: str, codigo_postal: str, 
    ...
    """
    # FIX 2026-02-01: Also catch TypeError for None values
    try:
    ...
    if HAS_PYTZ:
        ...
        tz = pytz.timezone(get_timezone_for_cp(codigo_postal))
        cfdi_dt = tz.localize(cfdi_dt)
        diff = abs((current - cfdi_dt).total_seconds() / 60)
    else:
        diff = abs((datetime.now() - cfdi_dt).total_seconds() / 60)
...
class MexicoTimezone:
    """
    Helper class para manejar zonas horarias de México.
    """
    ...
    def get_all_zones(cls) -> list:
        """Retorna todas las zonas horarias de México."""
        return [cls.TIJUANA, cls.MAZATLAN, cls.CHIHUAHUA, cls.CDMX, cls.CANCUN]
