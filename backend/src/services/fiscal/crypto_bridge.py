...
class CryptoBridge:
    ...
    def _ensure_table(self):
        ...
        self.core.db.execute_write("""
            CREATE TABLE IF NOT EXISTS crypto_conversions (
                ...
                amount_mxn DECIMAL(15,2) NOT NULL,  -- FIX 2026-02-01: PostgreSQL
                amount_usd DECIMAL(15,2) NOT NULL,  -- FIX 2026-02-01: PostgreSQL
                stablecoin TEXT NOT NULL,
                wallet_address TEXT,
                exchange_rate DECIMAL(15,4),  -- FIX 2026-02-01: PostgreSQL
        ...
        self.core.db.execute_write("""
            CREATE TABLE IF NOT EXISTS cold_wallets (
                ...
                stablecoin TEXT NOT NULL,
                balance_usd DECIMAL(15,2) DEFAULT 0,  -- FIX 2026-02-01: PostgreSQL
                last_updated TEXT,
                notes TEXT
            )
        """)
    ...
    def get_available_for_conversion(self) -> Dict[str, Any]:
        ...
        serie_b = list(self.core.db.execute_query("""
            ...
            FROM sales
            WHERE serie = 'B' AND EXTRACT(YEAR FROM timestamp::timestamp)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
        """, (str(year),)))
        ...
        converted = list(self.core.db.execute_query("""
            ...
            WHERE EXTRACT(YEAR FROM created_at::timestamp)::TEXT = %s AND status = 'completed'  -- FIX 2026-02-01: PostgreSQL
        """, (str(year),)))
        ...
        expenses = list(self.core.db.execute_query("""
            ...
            FROM cash_expenses
            WHERE EXTRACT(YEAR FROM expense_date::date)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
        """, (str(year),)))
    ...
    def _register_cover_expense(self, amount: float, description: str):
        ...
        try:
            ...
            pass  # Table not available or constraint violation
