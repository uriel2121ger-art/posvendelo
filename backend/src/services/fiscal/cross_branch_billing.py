...
class CrossBranchBilling:
    ...
    def get_rfc_load_status(self) -> Dict[str, Any]:
        ...
        try:
            rfcs = list(self.core.db.execute_query("""
                ...
                FROM invoices
                WHERE EXTRACT(YEAR FROM fecha)::TEXT = EXTRACT(YEAR FROM NOW())::TEXT  -- FIX 2026-02-01: PostgreSQL
                GROUP BY rfc
            """))
    ...
    def process_cross_invoice(self,
        ...
        try:
            ...
            """, (target_rfc, sale_id), commit=True)
    ...
    def _ensure_tables(self):
        ...
        self.core.db.execute_query("""
            CREATE TABLE IF NOT EXISTS cross_invoices (
                ...
                target_rfc TEXT,
                cross_concept TEXT,
                amount DECIMAL(15,2),  -- FIX 2026-02-01: PostgreSQL
                timestamp TEXT
            )
        """, commit=True)
