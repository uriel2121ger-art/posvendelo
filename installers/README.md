...
```bash
bash installers/linux/install-posvendelo.sh
```

Con URL y token explícitos (pruebas o entornos custom):

```bash
bash installers/linux/install-posvendelo.sh --cp-url https://posvendelo.com --install-token TOKEN
```

Opcional:

```bash
bash installers/linux/install-posvendelo.sh --cp-url http://localhost:9090 --install-token TOKEN --api-port 8002 --db-port 15434
```
...
```bash
cd /path/to/project
bash installers/linux/reinstall-posvendelo.sh --cp-url http://localhost:9090 --install-token TU_TOKEN
# Con directorio custom:
bash installers/linux/reinstall-posvendelo.sh --cp-url http://localhost:9090 --install-token TU_TOKEN --dir "$HOME/.posvendelo"
```

Actualizar el backend (cuando publiques una nueva imagen en GHCR):

```bash
cd ~/.posvendelo && ./actualizar.sh
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\installers\windows\Install-Posvendelo.ps1 -CpUrl https://posvendelo.com -InstallToken TOKEN
```

Opcional:

```powershell
powershell -ExecutionPolicy Bypass -File .\installers\windows\Install-Posvendelo.ps1 -CpUrl http://localhost:9090 -InstallToken TOKEN -ApiPort 8002 -DbPort 15434
```

Si usarás certificado autofirmado controlado, puedes importar confianza durante la instalación:

```powershell
powershell -ExecutionPolicy Bypass -File .\installers\windows\Install-Posvendelo.ps1 -CpUrl http://localhost:9090 -InstallToken TOKEN -PublisherCertPath ".\posvendelo-selfsigned-codesign.cer"
```

Si el script instala Docker Desktop por primera vez, despues de abrir Docker Desktop continua con:

```powershell
powershell -ExecutionPolicy Bypass -File .\installers\windows\Continue-Install.ps1 -StateFile "C:\ProgramData\POSVENDELO\install-state.json"
```
...
- **Linux / Raspberry Pi:** El instalador solo levanta el backend (Docker). Use el icono del escritorio o del menú; si la app no está instalada, descargue el .deb desde la web (en Raspberry Pi use el .deb para **arm64**, no el de amd64). Luego abra "POSVENDELO" desde el menú o ejecute `posvendelo`. Si la app no abre, ejecútela desde terminal (`posvendelo`) para ver mensajes de error.
- **Windows:** Use el icono del escritorio; si la app no está instalada, descargue el .exe desde la web. Ejecute "POSVENDELO" desde el menú Inicio. Si no inicia, ejecútela desde una consola para ver errores.
...
- Ambos generan `posvendelo-agent.json` en el directorio de instalacion para que la app desktop y el agente local compartan el mismo contrato de bootstrap.
...
- `posvendelo-agent.json`: contrato compartido entre instalador, agente local y app desktop
...
1. `bash installers/linux/install-posvendelo.sh --cp-url <URL_CONTROL_PLANE> --install-token <TOKEN>`
2. Comprobar que se generen en el directorio de instalación: `.env`, `docker-compose.yml`, `posvendelo-agent.json`, `INSTALL_SUMMARY.txt`
3. Opcional: `docker compose up -d` y verificar `http://127.0.0.1:<puerto>/health`

**Windows (VM o runner de CI):**

1. `powershell -ExecutionPolicy Bypass -File .\installers\windows\Install-Posvendelo.ps1 -CpUrl <URL> -InstallToken <TOKEN>`
