...
class CFDIPDFGenerator:
    ...
    async def generate(
        ...
        # Header with logo
        elements.extend(await self._build_header(cfdi_data, fiscal_config, logo_path, title_style))
        
        # Emisor/Receptor info
        elements.extend(await self._build_parties_info(cfdi_data, fiscal_config, styles))
        
        # Items table
        elements.extend(await self._build_items_table(cfdi_data, styles))
        
        # Totals
        elements.extend(await self._build_totals(cfdi_data, styles))
        
        # QR Code and digital seal
        elements.extend(await self._build_footer(cfdi_data, fiscal_config, styles))
    ...
    async def _build_header(self, cfdi_data, fiscal_config, logo_path, title_style):
    ...
    async def _build_parties_info(self, cfdi_data, fiscal_config, styles):
    ...
    async def _build_items_table(self, cfdi_data, styles):
        """Build items table."""
        elements = []
    ...
    async def _build_totals(self, cfdi_data, styles):
    ...
    async def _build_footer(self, cfdi_data, fiscal_config, styles):
...
async def generate_cfdi_pdf(cfdi_id: int, core, logo_path: Optional[str] = None) -> str:
    """
    ...
    await generator.generate(cfdi_data, fiscal_config, str(pdf_path), logo_path)
