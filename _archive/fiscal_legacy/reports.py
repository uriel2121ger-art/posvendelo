...
Args:
        ...
        result = await self.core.db.execute_query(sql, tuple(params))
    ...
    async def generate_customer_report(self, customer_rfc: str) -> str:
        """Generate report for specific customer."""
        sql = "SELECT * FROM cfdis WHERE rfc_receptor = %s ORDER BY fecha_emision DESC"
        result = await self.core.db.execute_query(sql, (customer_rfc,))
    ...
    async def get_statistics(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        result = await self.core.db.execute_query(sql, (start_date, end_date))
