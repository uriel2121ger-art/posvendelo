...
class PredictiveExtraction:
    ...
    def analyze_available(self) -> Dict[str, Any]:
        ...
        serie_b = list(self.core.db.execute_query("""
            ...
            FROM sales
            WHERE serie = 'B' AND date(timestamp) >= %s
        """, (month_start,)))
    ...
    def generate_extraction_plan(self, target_amount: float) -> Dict[str, Any]:
        ...
        days_needed = int(target_amount / daily_amount) + 1 if daily_amount > 0 else 1
        ...
        return {
            ...
            'plan': plan,
            'contracts': contracts,
            'message': f'Plan de {len(plan)} días generado. Retiro promedio de ${target_amount/len(plan):,.0f}/día' if plan else 'No se generó plan'
        }
    ...
    def get_optimal_daily_amount(self) -> Dict[str, Any]:
        ...
        daily_sales = list(self.core.db.execute_query("""
            SELECT date(timestamp) as day, COALESCE(SUM(total), 0) as total
            FROM sales
            WHERE serie = 'B' AND date(timestamp) >= %s
