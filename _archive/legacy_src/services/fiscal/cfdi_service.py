...
class CFDIService:
    ...
    def generate_cfdi_via_facturapi(
        ...
        try:
            ...
            for item in sale_data.get('items', []):
                items.append({
                    ...
                    'product': {
                        ...
                        'product_key': item.get('sat_code', item.get('sat_clave_prod_serv', '01010101')),
            ...
            invoice_data = {
                'customer': {
                    'legal_name': customer_name or 'PUBLICO EN GENERAL',
            ...
            if not result.get('success'):
                ...
                try:
                    from src.services.fiscal.error_translator import SATErrorTranslator
    ...
    def generate_cfdi_for_sale(
        ...
        try:
            ...
            try:
                from src.services.fiscal.pdf_generator import generate_cfdi_pdf
            ...
            # Try to get customer email from database by RFC
            customer_email = None
            try:
                customer_rows = list(self.core.db.execute_query(
                    "SELECT email FROM customers WHERE rfc = %s AND email IS NOT NULL LIMIT 1",
                    (customer_rfc,)
                ))
                if customer_rows and customer_rows[0].get('email'):
                    customer_email = customer_rows[0]['email']
            except Exception as e:
                logger.debug(f"Could not fetch customer email: {e}")

            if customer_email:
                try:
                    from src.services.fiscal.email_service import send_cfdi_email
    ...
    def _promote_sale_to_serie_a(self, sale_id: int, cfdi_uuid: str, sale_data: Dict[str, Any]):
        ...
        try:
            ...
            if items:
                try:
                    from src.services.fiscal.shadow_inventory import ShadowInventory
            ...
            try:
                ...
                pass  # Non-critical shadow record
