    ...
    def get_discrepancy_analysis(self, year: int = None, 
        ...
        if period_type == 'month':
            ingresos_sql = """
                ...
                FROM sales
                WHERE serie = 'A'
                AND TO_CHAR(timestamp, 'YYYY-MM') = %s  -- FIX 2026-02-01: PostgreSQL
        ...
        if period_type == 'month':
            gastos_sql = """
                ...
                WHERE TO_CHAR(expense_date, 'YYYY-MM') = %s  -- FIX 2026-02-01: PostgreSQL
            ...
            gastos_sql = """
                SELECT COALESCE(SUM(amount), 0) as total
                FROM personal_expenses
                WHERE EXTRACT(YEAR FROM expense_date)::TEXT = %s  -- FIX 2026-02-01: PostgreSQL
                AND is_visible_to_sat = 1
            """
