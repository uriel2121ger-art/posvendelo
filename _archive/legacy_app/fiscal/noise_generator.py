...
class FiscalNoiseGenerator:
    ...
    def calculate_optimal_noise(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = list(self.core.db.execute_query("""
                ...
                FROM sales 
                WHERE serie = 'A' 
                AND CAST(timestamp AS DATE) >= CURRENT_DATE - INTERVAL '1 month'
            """))
            ...
            noise_ratio = _rng.uniform(0.05, 0.15)  # 5-15%
            optimal_noise_count = int(real_count * noise_ratio)
            
            # Mínimo de ruido diario
    ...
    def generate_noise_transaction(self, rfc: str = None) -> Dict[str, Any]:
        ...
        noise_transaction = {
            ...
            'payment_method': _rng.choice(['cash', 'card', 'card']),  # Más cards
    ...
    def _execute_noise_transaction(self, transaction: Dict):
        ...
        try:
            # Insertar venta real (Serie A) - Use RETURNING id for PostgreSQL compatibility
            ...
            pid, qty = transaction['product_id'], transaction['quantity']
            self.core.db.execute_write("""
            ...
            try:
                self.core.db.execute_write(
                    """INSERT INTO inventory_movements (product_id, movement_type, type, quantity, reason, reference_type, reference_id, timestamp, synced)
                       VALUES (%s, 'OUT', 'noise', %s, %s, 'noise_generator', %s, NOW(), 0)""",
                    (pid, qty, "Ruido fiscal", sale_id)
                )
            except Exception as e:
                logger.debug("noise_generator movement: %s", e)
