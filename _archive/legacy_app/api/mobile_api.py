...
try:
    ...
    class _NoOpLimiter:
        """No-op fallback when slowapi is not installed."""
...
async def approve_merma(request: MermaApprovalRequest, user: Dict = Depends(verify_token)):
    ...
    if user.get('role') not in ('admin', 'manager', 'owner', 'gerente', 'dueño'):
        raise HTTPException(status_code=403, detail="Sin permisos para aprobar mermas")

    core = get_core()

    # Verify merma exists and is pending before updating
    existing = list(core.db.execute_query(
        "SELECT id, status FROM loss_records WHERE id = %s", (request.merma_id,)
    ))
    if not existing:
        raise HTTPException(status_code=404, detail="Merma no encontrada")
    if existing[0].get('status') not in ('pending', None, ''):
        raise HTTPException(status_code=400, detail=f"Merma ya fue procesada: {existing[0].get('status')}")
    ...
    core.db.execute_write(
        """UPDATE loss_records
           ...
           WHERE id = %s AND (status = 'pending' OR status IS NULL OR status = '')""",
...
async def update_price(
    ...
    if user['role'] not in ('admin', 'manager', 'owner', 'gerente', 'dueño'):
...
async def get_executive_dashboard(user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        logger.error("Error en executive-dashboard: %s", e, exc_info=True)
        return {
...
async def get_turn_status(user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        logger.error("Error en turn-status: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="Error interno del servidor")
...
async def send_notification_to_pos(request: NotificationRequest, user: Dict = Depends(verify_token)):
    ...
    # Use correct column names matching the real schema (body, notification_type, sent)
    core = get_core()
    try:
        core.db.execute_write(
            """INSERT INTO remote_notifications
               (title, body, notification_type, user_id, created_at, sent)
               VALUES (%s, %s, %s, %s, %s, 0)""",
            (request.title, request.message, request.priority or "normal",
             user['sub'], datetime.now().isoformat())
        )
        return {"success": True, "message": "Notificación enviada al POS"}
    except Exception as e:
        logger.error("Error sending notification via deprecated endpoint: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="Error enviando notificación. Use /api/v1/remote/notification")
...
async def get_pending_notifications(user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        if notifications:
            core.db.execute_write("UPDATE remote_notifications SET sent = 1 WHERE sent = 0")
        ...
        return {
            ...
            "notifications": [{
                ...
                "title": n.get('title', ''),
                "message": n.get('body', ''),
                "priority": n.get('notification_type', 'normal'),
                "timestamp": n.get('created_at', '')
            } for n in notifications]
        }
    except Exception as e:
        logger.error("Error en pending-notifications: %s", e, exc_info=True)
...
async def remote_change_price(request: PriceChangeRequest, user: Dict = Depends(verify_token)):
    ...
    core.db.execute_write(
        ...
        ('REMOTE_PRICE_CHANGE', 'product', product['id'], user['sub'],
         json.dumps({"old_price": old_price, "new_price": request.new_price, "reason": request.reason or "PWA Remote"}),
         datetime.now().isoformat())
    )
...
async def get_system_status(user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        logger.error("Error en check-pos-status: %s", e, exc_info=True)
        return {
            "pos_online": True,
            "error": "Error verificando status",
            "timestamp": datetime.now().isoformat()
        }
...
async def list_products(
    ...
    try:
        ...
        logger.error("Error en list-products: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="Error interno del servidor")
...
async def get_product(product_id: int, user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        logger.error("Error en get-product: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="Error interno del servidor")
    ...
    try:
        ...
        core.db.execute_write(
            ...
            ('PRODUCT_UPDATED', 'product', product_id, user['sub'], 
             json.dumps({"source": "PWA", "sync_all": request.sync_all_branches}), now)
        )
    ...
    try:
        ...
        core.db.execute_write(
            ...
            ('PRODUCT_DELETED', 'product', product_id, user['sub'], 
             json.dumps({"name": product_name, "source": "PWA"}), now)
        )
        ...
        logger.error("Error en delete-product: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="Error interno del servidor")
...
async def get_sat_code_info(code: str, user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        logger.error("Error en sat-lookup %s: %s", code, e, exc_info=True)
        return {
            "success": False,
            "code": code,
            "description": "No existe en el catálogo",
            "error": "Error buscando en catálogo SAT"
        }
