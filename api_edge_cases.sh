...
if [ -z "$API_PASSWORD" ]; then
  echo "Define API_PASSWORD para ejecutar los edge cases."
  exit 1
fi

LOGIN_PAYLOAD=$(printf '{"username":"%s","password":"%s"}' "$API_USER" "$API_PASSWORD")
TOKEN=$(curl -s -X POST "$BASE_URL/api/v1/auth/login" -H "Content-Type: application/json" -d "$LOGIN_PAYLOAD" | grep -o '"access_token":"[^"]*' | cut -d'"' -f4)
if [ -z "$TOKEN" ]; then
    echo "Fallo al obtener TOKEN"
    curl -s -X POST "$BASE_URL/api/v1/auth/login" -H "Content-Type: application/json" -d "$LOGIN_PAYLOAD"
    exit 1
fi
...
CID=$(curl -s -X POST "$BASE_URL/api/v1/customers/" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name": "Monkey Victim", "rfc": "XAXX010101000", "email": "a@a.com", "phone": "123"}' | grep -o '"id":[0-9]*' | head -1 | cut -d':' -f2)
...
curl -s -X DELETE "$BASE_URL/api/v1/customers/$CID" -H "Authorization: Bearer $TOKEN"
